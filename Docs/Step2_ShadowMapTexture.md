...
### 変更① `Engine/Direct3D.h` に宣言を追加する

`GetLightProjectionMatrix()` の直後に追記します。
...
### 変更② `Engine/Direct3D.cpp` の namespace 内にリソース変数を追加する

`lightPosition` の宣言の近く（namespace Direct3D の `{` の中）に追記します。
...
### 変更③ `Engine/Direct3D.cpp` に `InitShadowMap()` と `GetShadowMapSRV()` を追加する

ファイル末尾（`GetLightProjectionMatrix()` の後）に追記します。
...
### 変更④ `Direct3D::Initialize()` の中から `InitShadowMap()` を呼ぶ
...
### 変更⑤ `Direct3D::Release()` にシャドウマップの解放を追加する

`SAFE_RELEASE(pRenderTargetView);` の直前に追記します。
...
### 変更⑥ `Stage.cpp` の ImGui デバッグ表示に確認用テキストを追加する

Step1 のデバッグ表示ブロックの直後に追記します。
